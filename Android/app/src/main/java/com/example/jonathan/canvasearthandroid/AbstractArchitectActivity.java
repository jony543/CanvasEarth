 ...
 */
public abstract class AbstractArchitectActivity extends Activity implements ArchitectViewHolderInterface {
            ...
            this.locationListener = new LocationListener() {
                ...
                public void onLocationChanged(final Location location) {
                    ...
                    if (location != null) {
                        ...
                        AbstractArchitectActivity.this.lastKnownLocaton = location;
                        if (AbstractArchitectActivity.this.architectView != null) {
                            // check if location has altitude at certain accuracy level & call right architect method (the one with altitude information)
                            if (location.hasAltitude() && location.hasAccuracy() && location.getAccuracy() < 7) {
                                AbstractArchitectActivity.this.architectView.setLocation(location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy());
                            } else {
                                AbstractArchitectActivity.this.architectView.setLocation(location.getLatitude(), location.getLongitude(), location.hasAccuracy() ? location.getAccuracy() : 1000);
                            }
                        }
                    }
                }
                ...
                public void run() {
                    ...
                    while (lastKnownLocaton == null && !isFinishing()) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                Toast.makeText(AbstractArchitectActivity.this, R.string.location_fetching, Toast.LENGTH_SHORT).show();
                            }
                        });
